  ...
  return (
    <div className="p-4 sm:p-6">
      <p className="text-sm text-muted-foreground mb-4">
        Share this link to send a copy of your project's script and assets. The recipient will need to regenerate images
        and videos.
      </p>
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < generateOutputs; i++) {
        ...
        console.log(
          `✅ API Call ${i + 1} SUCCESS - Got image: ${imageUrl ? imageUrl.substring(0, 50) + "..." : "null"}`,
        )
        ...
        <div className="space-y-4 pb-4">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <Select
                value={generateOutputs.toString()}
                onValueChange={(value) => setGenerateOutputs(Number.parseInt(value))}
              >
...
export const StoryBuilder: React.FC<StoryBuilderProps> = ({
  ...
  return (
    <div className="flex flex-col h-screen text-foreground ">
      ...
      {analysisError && (
        <div className="bg-destructive/20 text-destructive p-2 sm:p-3 text-center  text-xs sm:text-sm">
          {analysisError}
        </div>
      )}
              ...
              {project.storyboard.length > 0 ? (
                ...
                <div className="text-center py-12 sm:py-20">
                  ...
                  <p className="text-sm sm:text-base text-muted-foreground mt-2 px-4">
                    Analyze your script or add a scene to get started.
                  </p>
                  <Button onClick={handleAddScene} className="mt-4">
                    Add Scene
                  </Button>
                </div>
              )}
