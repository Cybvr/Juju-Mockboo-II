...
export const createStickyNoteClass = async () => {
  if (!fabric) {
...
export let StickyNote: any

export const initializeStickyNote = async () => {
  if (!StickyNote) {
    StickyNote = await createStickyNoteClass()
    ...
    fabric.StickyNote.fromObject = (obj: any, callback: any) => {
      const note = new StickyNote(obj.text, {
        ...obj,
        left: obj.left,
        top: obj.top,
      })
